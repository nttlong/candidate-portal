...
var convertDateFields = (listOfFields, data) => {
    ...
    for (var i = 0; i < listOfFields.length; i++) {
        ...
        if (!isNull(val)) {
            ...
            setValue(data, listOfFields[i], null);
        }
    }
    return data;
}
...
var checkRequireFieldsList = (listOfFields, data) => {
    var ret = [];
    for (var i = 0; i < listOfFields.length; i++) {
        for (var j = 0; j < data.length; j++) {
            var val = getValue(data[j], listOfFields[i]);
            if (isNull(val)) {
                ret.push({
                    errorType: listOfFields[i] + "IsEmpty",
                    field: listOfFields[i],
        ...
        try {
            ...
            convertToArrayTable(list, (err, result) => {
                ...
                for (var i = 0; i < cols.length; i++) {
                    ...
                    sheet.getCell(1, i + 1).value = {
                        'richText': [
                            { 'font': { 'bold': true, 'size': 12, 'color': { 'argb': 'ffffff' }, 'name': 'Calibri', 'family': 2, 'scheme': 'minor' }, 'text': cols[i] }
                        ]
                    };
...
module.exports = {
    objectID: objectID,
    checkRequireFieldsList: checkRequireFieldsList,
